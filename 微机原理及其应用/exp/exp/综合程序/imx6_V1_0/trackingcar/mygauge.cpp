...
myGauge::myGauge(QWidget *parent) :
        QWidget(parent)
{
    this->initVariables();
}

void myGauge::initVariables()
{
    QFont font;
     font.setPointSize(6);
    this->setFont(font);
   ...
   m_innerRadius=m_outerRadius/8*7;
    ...
    m_colorCircleRadius=m_outerRadius/8*3;
...
void myGauge::drawOuterCircle(QPainter *painter)
{
    painter->save();
...
void myGauge::drawInnerCircle(QPainter *painter)
{
    painter->save();
...
void myGauge::drawMark(QPainter *painter)
{
    painter->save();
...
void myGauge::drawCoverBall(QPainter *painter)
{
    painter->save();
...
void myGauge::drawTextRect(QPainter *painter)
{
    painter->save();
    qreal rectWidth=m_coverCircleRadius/3;
    ...
    painter->drawRoundRect(textRect,15,15);//ANGLE,ANGLE);
    ...
    QString strValue;
    strValue=tr("%1km/h").arg(m_value);
    painter->drawText(textRect,Qt::AlignHCenter|Qt::AlignVCenter,strValue);
    painter->restore();
}

void myGauge::drawCoverCircle(QPainter *painter)
{
    painter->save();
...
void myGauge::drawColorPie(QPainter *painter)
{
    painter->save();
    ...
    greenGradient.setColorAt(0.8,QColor(0,230,0));
...
void myGauge::drawIndicator(QPainter *painter)
{
    painter->save();
    ...
    QPointF bottomLeftPot(-m_coverBallRadius/5,0);
    QPointF bottomRightPot(m_coverBallRadius/5,0);
    painter->setPen(Qt::NoPen);
...
void myGauge::resetVariables(QPainter *painter)
{
    m_outerRadius=width()>height() ? height()/2: width()/2;
    m_innerRadius=m_outerRadius/8*7;
    m_coverCircleRadius=m_outerRadius/8*4-5;
    m_colorCircleRadius=m_outerRadius/8*4.5;

    m_center=rect().center();
}

void myGauge::setValue(qreal value)
{
    if(value>m_value)
    {
        m_bReverse=false;
